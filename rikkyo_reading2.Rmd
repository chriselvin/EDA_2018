...
###### Report homework assignment scores to students.
...
###### QUESTION What was the average number of books the students read?

```{r}
mean(table(reading$Nickname))
```

###### QUESTION What was the average score per book?
...
###### COMMENT Some students edited their work for the better. Overall, the pattern seemed to be of consistency over time, apart from the last few weeks at Tokyo when the deadline loomed for some.

###### QUESTION What was the difference between the two teacher's assessments?

```{r}
table(reading$Teacher_Assessment, reading$Teacher_Assessment_2)
```

###### COMMENT Mostly students scored higher for the second assessment. The two principal factors were students editing (to improve), and teacher marking error.
...
```{r}
final_scores <- select(reading, Nickname, Campus, Teacher_Assessment,
...
###### COMMENT The second assessment will be their final assessment. They did a great job!
